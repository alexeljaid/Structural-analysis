...
FixRotNodes = [1 0 0 1; 1*NumElemSM+1 0 0 1; 2*NumElemSM+1 0 0 1]; % not sure if we need rotation restrictions here!!!!!
...
Sigma = Axial/(LengthSM*sin(alpha)); % Option 2

% % Loading in the y direction
% Sigma = P/(LengthSM*b); % Option 1
% Axial = max(max(abs(BeamEndForcesLAS(:,[1 4])))); % Option 2
% Sigma = 3*Axial/(2*LengthSM*sin(alpha)); % Option 2
