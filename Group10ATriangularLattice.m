...
t = sqrt(2)/4;   % mm % relative density = 0.05
% t = sqrt(2)/2;   % mm % relative density = 0.1
% t = 3*sqrt(2)/4; % mm % relative density = 0.15
% t = sqrt(2);     % mm % relative density = 0.2
% t = 5*sqrt(2)/4; % mm % relative density = 0.25
b = 1; % unit value (mm)
E = 2000; % MPa
...
NodePoints(1,:) = [0 0]; % node 1
NodePoints(1*NumElemSM+1,:) = [LengthSM*cos(alpha) -LengthSM*sin(alpha)]; % node 9
NodePoints(2*NumElemSM+1,:) = [-LengthSM*cos(alpha) -LengthSM*sin(alpha)]; % node 17
...
LoadedNode = 2*NumElemSM+1; % node 17
...
% For triangular lattices, we release all the rotation restrictions
...
% % Fix rotations just as a comparison

% FixRotNodes = [1 0 0 1; 1*NumElemSM+1 0 0 1; 2*NumElemSM+1 0 0 1]; % rotation fixed 
...
% Calculate the stress using Option 1 or Option 2
% Calculate the strain
% Calculate Young's modulus by dividing the stress by the strain

% Loading in the x direction
Sigma = P/(LengthSM*sin(alpha)*b); % Option 1
% Axial = max(max(abs(BeamEndForcesLAS(:,[1 4])))); % Option 2
% Sigma = Axial/(LengthSM*sin(alpha)*b); % Option 2
Epsilon = NodeDisps(1+2*NumElemSM,1)/LengthSM;
EX = Sigma/Epsilon;

% % Loading in the y direction
% Sigma = P/(LengthSM*b); % Option 1
% % Axial = max(max(abs(BeamEndForcesLAS(:,[1 4])))); % Option 2
% % Sigma = sqrt(3)*Axial/(LengthSM*b); % Option 2
